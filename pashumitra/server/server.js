...
// Cron job route - can be called manually or by external cron service
app.get('/api/cron/injection-notifications', async (req, res) => {
  try {
...
app.post('/api/trigger/injection-notifications', async (req, res) => {
  try {
    const result = await checkAndSendInjectionNotifications();
    res.json(result);
  } catch (error) {
...
cron.schedule('*/15 * * * *', async () => {
  console.log('Running scheduled injection notification check...');
  await checkAndSendInjectionNotifications();
});

// Start server
app.listen(PORT, () => {
  console.log(`PashuMitra server running on port ${PORT}`);
  console.log(`Health check: http://localhost:${PORT}/api/health`);
  console.log(`Manual trigger: http://localhost:${PORT}/api/trigger/injection-notifications`);
  console.log(`Cron endpoint: http://localhost:${PORT}/api/cron/injection-notifications`);
});
